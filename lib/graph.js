...
function show_node(query) {
  ...
  vis.selectAll(".node")
    .classed("marked", function(d) {
      return d.id.indexOf(query.toLowerCase()) >= 0 ||
             (d.hostname && d.hostname.toLowerCase().indexOf(query.toLowerCase()) >= 0)
    })
}
...
function goto_node(d) {
  if (d3.event.defaultPrevented || d.flags.client)
    return
...
function show_node_info(d) {
  window.location = "node.html?id=" + d.id
}
...
function wilder_scheiß(data) {
  ...
  var lat = nodes.map(function(d) { return d.location.latitude })
  var lon = nodes.map(function(d) { return d.location.longitude })
  ...
  nodes.forEach(function(d) {
    ...
    d.x = (d.location.longitude - min_lon) * scale_x
    d.y = (d.location.latitude - min_lat) * scale_y
  })
...
function update() {
  ...
  var geodot = labelEnter.filter(function(d) { return d.location })
  ...
  label.selectAll("text.name")
      .text(function(d) {
        return d.hostname? shorten(d.hostname, 17) : d.id
      })
