...
function update(data) {
  var nodes = data.nodes.filter( function(d) {
    return d.location != null && (d.location.latitude != 0 || d.location.longitude != 0)
  })
  nodes.forEach(function(node) { node.geo = [node.location.latitude, node.location.longitude]})
  ...
  } else if(ffmapConfig.extent) {
    map.fitBounds(ffmapConfig.extent)
  } else {
  ...
  var circles = new_nodes.append("circle")
           ...
           .attr("fill", function (d) {
             return d.flags.online?(is_legacy_node(d)?"rgba(255, 55, 55, 1.0)":"rgba(0, 255, 0, 0.8)"):"rgba(128, 128, 128, 0.2)"
           })
           .on("click", function(n) { window.location.href = "graph.html#" + n.id })
           .append("title").text( function (n) { return n.hostname?n.hostname:n.id })
  ...
  */
  ...
  label_g.append("g").each( function (d) {
    ...
    d.nodes.forEach( function (n, i) {
      var name = shorten(n.hostname, 17) || n.id
