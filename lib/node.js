function nodedetails(container, fn) {
  ...
  function update(data) {
      ...
      var filename = ffmapConfig.nodes_json.replace(/\/[^\/]*\/?$/, '') + '/' + id.replace(/:/g, '') + '.rrd';
      var node;
      ...
      var details = {
          ...
          'Modell': (node.hardware && node.hardware.model)? document.createTextNode(node.hardware.model) : unknown.cloneNode(),
          'Firmware-Version': (node.software && node.software.firmware)? document.createTextNode(node.software.firmware.release) : unknown.cloneNode(),
          'Koordinaten': get_location_link(node)? (function() {
              var link = document.createElement('a');
              link.href = get_location_link(node);
              link.textContent = node.location.latitude + ',' + node.location.longitude;
              return link;
          })() : unknown.cloneNode()
      };
