function nodedetails(container, fn) {
  ...
  function update(data) {
      ...
      var details = {
          'Firmware-Version': (node.software && node.software.firmware)? node.software.firmware.release : 'unbekannt'
      };
      for(var caption in details) {
          var li = document.createElement('li');
          li.textContent = caption + ': ' + details[caption];
          document.getElementById('nodedetails').appendChild(li);
      }
      nodecaption.textContent = node.hostname + ' (' + id + ')';
